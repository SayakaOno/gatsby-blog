...
import { pageExist, getCorrespondingPath } from '../../../utils/get-corresponding-link';
...
const LanguageSwitcher = ({ link, language, path }) => {
	...
	const categoriesPathList = path.includes('/category/') ? useCategoriesPathList() : null;
	...
	useEffect(() => {
		...
		if (path.includes('/category/') || path.includes('/tag/')) {
			if (pageExist(path, path.includes('/tag/') ? tagsPathList : categoriesPathList)) {
	...
	const otherLanguagePath = (language, currentPath) => {
		if (currentPath.includes('/page/')) {
			...
			return '/ja';
		} else if (currentPath.includes('/category/') || currentPath.includes('/tag/')) {
