...
const PaginationBox = ({ currentPage, totalPage, language, dates }: Props) => {
	...
	const getLinksWidth = () => {
		return Math.min(paginationUlRef.current.offsetWidth, paginationBoxRef.current.offsetWidth - 72);
	};
	...
	const onScroll = (arrow) => {
		...
		scrollId = setInterval(() => (paginationListRef.current.scrollLeft += value), 1);
	};
	...
	const setDisplayArrows = () => {
		if (paginationUlRef.current.offsetWidth < paginationBoxRef.current.offsetWidth - 72) {
	...
	const scrollable = (arrow) => {
		...
		return (
			...
			paginationUlRef.current.offsetWidth - paginationListRef.current.offsetWidth
		);
	};
	...
	const displayDate = (i, event) => {
		...
		toolTipRef.current.style.left = clickedLinkCoordsLeft - bodyCoordsLeft + 'px';
	};
	...
	const renderToolTip = () => {
		return (
			<div className={styles['pagination-box__body__tooltip-container']}>
				<div ref={toolTipRef} className={styles['pagination-box__body__tooltip']} />
			</div>
		);
	};
	...
	const renderLinkList = () => {
		return (
			<div onScroll={colorArrow} ref={paginationListRef} className={styles['pagination-box__body__links']}>
				<ul ref={paginationUlRef} className={styles['pagination-box__body__links__list']}>
		...
		for (let i = 0; i < totalPage; i++) {
			listItems.push(
				<li
					...
					className={styles[`pagination-box__body__links__list-item${i === currentPage ? '-active' : ''}`]}
				>
